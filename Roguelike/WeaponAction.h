...
public:
	...
	WeaponAction(std::string name, std::string logDescription, double actionProficiency, int range, double damageModifier,
		double accuracyModifier, double staminaCostModifier, double durabilityModifier, std::shared_ptr<VisualEffect> visualEffectOnExecution = nullptr, TargetType targetType = TargetType::IN_RANGE)
		:
